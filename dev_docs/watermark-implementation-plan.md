...
- [x] Создать `WatermarkDto` с поддержкой режимов `single` и `tile`
- [x] Добавить `watermark` в `TransformDto`
- [x] Добавить unit тесты для валидации DTO

### Этап 2: Обновление Controller
- [x] Изменить `process()` для поддержки нескольких файлов через `req.parts()`
- [x] Добавить обработку файла водяного знака
- [x] Добавить валидацию: если `transform.watermark` указан, файл `watermark` обязателен
- [x] Обработать ошибки (отсутствие файла, неподдерживаемый формат)

### Этап 3: Реализация в Service
- [x] Создать метод `applyWatermark()` в `ImageProcessorService`
- [x] Реализовать `createSingleWatermark()` для режима single
- [x] Реализовать `createTiledWatermark()` для режима tile
- [x] Реализовать `scaleWatermark()` для масштабирования
- [x] Интегрировать в метод `processStream()`
...
- [x] E2E тесты с PNG водяным знаком
- [x] E2E тесты с SVG водяным знаком
- [x] Тесты для edge cases (отсутствие файла, невалидные параметры)

### Этап 5: Документация
- [x] Обновить README.md с примерами использования
- [x] Добавить примеры cURL запросов для обоих режимов
- [x] Добавить предупреждение о преобразовании текста в кривые для SVG
- [x] Обновить Web UI для поддержки водяных знаков
- [x] Обновить roadmap (переместить из v1.1 в completed)
